...
// 초기 보드 설정 (10행 9열, 0-indexed)
// 초(CHO)가 하단, 한(HAN)이 상단
  ...
  switch (type) {
    ...
    case PIECES.CANNON: {
      ...
      for (const [dr, dc] of dirs) {
        ...
        while (inBounds(nr, nc)) {
          ...
          if (!jumped) {
            if (target) {
              if (target.type === PIECES.CANNON) break; // 포로 뛰어넘기 불가
              jumped = true;
            }
          } else {
            if (target) {
              if (target.team === enemy && target.type !== PIECES.CANNON) {
                moves.push([nr, nc]);
              }
              break;
            }
    ...
    case PIECES.HORSE: {
      ...
      for (const [dr1, dc1, dr2, dc2] of horseMoves) {
        ...
        const nr = row + dr1 + dr2, nc = col + dc1 + dc2;
...
export function findKing(board, team) {
  for (let r = 0; r < 10; r++) {
    for (let c = 0; c < 9; c++) {
      const p = board[r][c];
      if (p && p.type === PIECES.KING && p.team === team) return [r, c];
    }
  }
  return null;
}
...
export function isGameOver(board) {
  const choKing = findKing(board, TEAMS.CHO);
  const hanKing = findKing(board, TEAMS.HAN);
